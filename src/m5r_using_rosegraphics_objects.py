...
#            It lets pixel information go to the window so the window can display what is attached to it.
#
#     g. When is a RoseWindow close_on_mouse_click method call necessary?  Why?
#            It is needed when multiple windows must open in sequence, one at a time. Else, multiple would appear
#            at once and would all close immediately after rendering.
#
...
def example4():
    window = rg.RoseWindow(200,100)
    point10 = rg.Point(100, 50)
    point10.attach_to(window)
    window.render()
